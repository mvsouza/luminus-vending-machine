...
The `home-routes` handler in the `luminus-vending-machine.routes.home` namespace
...
The `home-page` function will in turn call the `luminus-vending-machine.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `luminus-vending-machine.handler` namespace:
...
Request middleware functions are located under the `luminus-vending-machine.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `luminus-vending-machine.dev-middleware` namespace found in
